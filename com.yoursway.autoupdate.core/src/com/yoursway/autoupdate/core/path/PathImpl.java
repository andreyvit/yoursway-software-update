...
public class PathImpl implements Path, Cloneable {
	...
	public static final PathImpl EMPTY = new PathImpl(EMPTY_STRING);
	...
	public static final PathImpl ROOT = new PathImpl(ROOT_STRING);
	...
	public static Path fromPortableString(String pathString) {
		...
		if (firstMatch <= 0)
			return new PathImpl().initialize(null, pathString);
		...
		if (pathString.indexOf(DEVICE_SEPARATOR) == -1) 
			return new PathImpl().initialize(devicePart, pathString);
		...
		return new PathImpl().initialize(devicePart, new String(chars, 0, writeOffset));
	}
	 ...
	 */
	PathImpl(String device, String path) {
		if (WINDOWS) {
			//convert backslash to forward slash
	 ...
	 * Private constructor.
	 */
	private PathImpl(String device, String[] segments, int _separators) {
	...
	public Path addTrailingSeparator() {
		...
		if (isEmpty()) {
			return new PathImpl(device, segments, HAS_LEADING);
		}
		return new PathImpl(device, segments, separators | HAS_TRAILING);
	}
	...
	public Path append(Path tail) {
		...
		PathImpl result = new PathImpl(device, newSegments, (separators & (HAS_LEADING | IS_UNC)) | (tail.hasTrailingSeparator() ? HAS_TRAILING : 0));
	...
	public Path append(String tail) {
		...
		if (tail.indexOf(SEPARATOR) == -1 && tail.indexOf("\\") == -1 && tail.indexOf(DEVICE_SEPARATOR) == -1) { //$NON-NLS-1$
			...
			return new PathImpl(device, newSegments, separators & ~HAS_TRAILING);
		}
		//go with easy implementation
		return append(new PathImpl(tail));
	}
	...
	public boolean isValidPath(String path) {
		PathImpl test = new PathImpl(path);
	...
	public Path makeAbsolute() {
		...
		PathImpl result = new PathImpl(device, segments, separators | HAS_LEADING);
	...
	public Path makeUNC(boolean toUNC) {
		...
		return new PathImpl(toUNC ? null : device, segments, newSeparators);
	}
	...
	public Path removeFirstSegments(int count) {
		...
		if (count >= segments.length) {
			return new PathImpl(device, NO_SEGMENTS, 0);
		}
	...
	public Path removeLastSegments(int count) {
		...
		if (count >= segments.length) {
			...
			return new PathImpl(device, NO_SEGMENTS, separators & (HAS_LEADING | IS_UNC));
		}
		Assert.isLegal(count > 0);
		int newSize = segments.length - count;
		String[] newSegments = new String[newSize];
	...
	public Path removeTrailingSeparator() {
		...
		return new PathImpl(device, segments, separators & (HAS_LEADING | IS_UNC));
	}
	...
	public Path setDevice(String value) {
		...
		return new PathImpl(value, segments, separators);
	}
